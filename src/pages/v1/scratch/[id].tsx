...
const ScratchCardPage = () => {
  ...
  const fixImageUrl = (url: string) => {
    if (!url) return '';
    return url
      .replace('raspa.ae', 'api.ultrapanel.shop')
  ...
  const fetchScratchCardData = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`https://api.ultrapanel.shop/v1/api/scratchcards/${id}`);
  ...
  const playGame = async (authToken: string): Promise<{ result: GameResult | null, errorMessage?: string }> => {
    ...
    try {
      setPlayingGame(true);
      const response = await fetch('https://api.ultrapanel.shop/v1/api/scratchcards/play', {
        method: 'POST',
        headers: {
